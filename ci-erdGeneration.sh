...
sudo mkdir -p /var/www/html/erd-buq
sudo chown -R $USER:$USER /var/www/html/erd-buq
...
&& wget https://raw.githubusercontent.com/OpenLMIS/openlmis-buq/master/docker-compose.erd-generation.yml -O docker-compose.yml \
&& (/usr/local/bin/docker-compose up &) \
&& sleep 90 \
&& sudo rm /var/www/html/erd-buq/* -rf \
&& sudo rm -rf output \
&& mkdir output \
&& chmod 777 output \
&& export COMPOSE_PROJECT_NAME_LOWER_CASE=`echo "$COMPOSE_PROJECT_NAME" | tr '[:upper:]' '[:lower:]'` \
&& (docker run --rm --network ${COMPOSE_PROJECT_NAME_LOWER_CASE//.}_default -v $WORKSPACE/erd/output:/output schemaspy/schemaspy:6.1.0 -t pgsql -host db -port 5432 -db open_lmis -s buq -u postgres -p p@ssw0rd -I "(data_loaded)|(schema_version)|(jv_.*)" -norows -hq &) \
&& sleep 90 \
&& /usr/local/bin/docker-compose down --volumes \
&& sudo chown -R $USER:$USER output \
&& mv output/* /var/www/html/erd-buq \
&& rm erd-buq.zip -f \
&& pushd /var/www/html/erd-buq \
&& zip -r $WORKSPACE/erd/erd-buq.zip . \
&& popd \
&& rmdir output \
&& rm .env \
